...
* **Dropped invalid records:** Removed any transactions that didn't have a valid `CustomerID`.
* **Excluded anomalies:** Filtered out negative quantities and prices to remove canceled orders and product returns.
* **Stripped noise:** Used Regular Expressions (RegEx) to filter out manual administrative records like postage fees.
* **Engineered RFM features:** Aggregated the data by customer to calculate their Recency, Frequency, and Monetary (RFM) scores, then grouped them into clear segments like *Champions*, *At Risk*, and *Lost*.
...
![Executive Dashboard Interface](dashboard_preview.png)
